...
这个自动文本摘要模型由Salesforce研究发表，通过一系列tricks提升模型概括长文本的能力。

针对长文本生成概括式（长）摘要在文本摘要领域是一项比较困难的任务，即使是过去最好的深度神经网络模型，在处理这项任务时，也会出现生成不通顺、重复词句等问题。为了提升文本摘要的生成质量，模型作者提出了基于**内注意力**（intra-attention mechanism）机制和**新的训练方法**的模型架构。通过内注意力机制，使模型能关注到已生成的词，从而避免上述问题。另一方面，新的训练方法结合了传统的监督式学习（teacher forcing）和强化学习。

模型：值得关注的点有两个，一是模型里应用了两套注意力机制，分别是经典的解码器-编码器注意力机制，和解码器内部的注意力机制。前者使解码器在生成结果时，能动态地、按需求地获得输入端的信息，后者则帮助解决了生成长句子时容易重复同一词句的问题。二是概述中提到的混合式学习目标。该学习目标包含了传统的最大似然，在语言建模等任务中是一个经典的训练目标，但对于文本摘要，仅仅考虑最大似然并不够。主要有两个原因，一是监督式训练时有参考“答案”，但投入应用、生成摘要时却没有，这样引起的后果是因为没有纠正，错误会积累，这个原因学术上称为exposure bias。另一个原因是，往往在监督式训练中，我们只提供一个参考摘要，训练时要求模型生成一模一样的摘要，然而对于一篇文本，往往可以有不同的总结，因此监督式学习的要求太过绝对。与此相反，用于评价生成摘要的ROUGE指标却能考虑进这一灵活性。所以一个很自然的想法是，利用强化学习将ROUGE指标加入训练目标。因此，最终的训练目标是最大似然和基于ROUGE指标的函数的加权平均，这两个子目标各司其职：最大似然承担了建立好的语言模型的责任，使模型生成可读性高的文本；ROUGE指标则允许摘要拥有更多的灵活性，同时针对ROUGE的优化也直接提升了模型的ROUGE评分。

构建一个好的模型，除了在架构上需要有创新，也需要一些小tricks，本文也不例外：

1. 使用指针处理未知词（OOV）问题
2. 共享解码器权重，加快训练时模型的收敛
3. 人工规则，规定不能重复出现连续的三个词

总结：深度学习+强化学习是一个很好的思路，需要在这方面探索更多的可能性。关于强化学习，可以进一步了解这篇论文用到的self-critical policy gradient training algorithm，相比其他的强化学习方法，它不需要额外的神经网络来支持学习，同时能获得更好的表现。我们在文本摘要这块可以借鉴这个思路。
